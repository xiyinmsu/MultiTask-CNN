...
/*template <typename Dtype>
class PCAPowerLayer : public NeuronLayer<Dtype> {
 public:
  explicit PCAPowerLayer(const LayerParameter& param)
      : NeuronLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "PCAPower"; }
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  virtual inline int ExactNumTopBlobs() const { return 1; }

  // PCA related functions written by Xi Yin
  bool exists_test (const std::string& name);
  Eigen::MatrixXd read_data_from_file(const std::string& name, const int channel);
  Eigen::MatrixXd compute_principle_component(const Eigen::MatrixXd& allSamples, const int channel);
  void save_princomp_to_file(const Eigen::MatrixXd& princomp, const std::string& name);
  void data_projection(const vector<Blob<Dtype>*>& bottom, const Eigen::MatrixXd& princomp);
 ...
 protected:
  ...
  Dtype nbasis_; // the number of principal components 
  vector<int> bottom_shape_;  // the shape of bottom[0]
  vector<int> top_shape_;
  vector<Dtype> convMax_ ; // covariance matrix
  vector<Dtype> princomp_; // first principal component
  bool train_;
}; */
