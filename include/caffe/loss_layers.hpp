...
// copied from softmaxwithloss layer to take a dynamic weight as a bottom 
template <typename Dtype>
class SoftmaxWithLossDynamicLayer : public LossLayer<Dtype> {
 public:
   /**
    * @param param provides LossParameter loss_param, with options:
    *  - ignore_label (optional)
    ...
    *    present; otherwise the loss is simply summed over spatial locations.
    */
  explicit SoftmaxWithLossDynamicLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "SoftmaxWithLossDynamic"; }
  virtual inline int ExactNumTopBlobs() const { return -1; }
  virtual inline int ExactNumBottomBlobs() const { return 3; }
...
/*template <typename Dtype>
class BinarySplitLossLayer : public LossLayer<Dtype> {
 public:
  explicit BinarySplitLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  ...
  virtual inline const char* type() const { return "BinarySplitLoss"; }

 protected:
  /// @copydoc BinarySplitLossLayer
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
